...
import { filterBarcodeTypes } from "../utilities/barcodeTypeUtils";
...
const EditTemplateModal = (props) => {
  ...
  useEffect(() => {
    const runandupdate = async () => {
      ...
      if (layoutData) {
        ...
        console.log(layoutData?.barcodeData)
        ...
        if (layout) {
          ...
          setBarcodeRejectStatus(
            comparewithId(barcodeRejectData, barcodeData?.barcodeOption)
          );
          setBarcodeType(
 ...
 const filteredBarcodeCategoryData =
    scanner?.name === "SR 11000"
      ? barcodeCategoryData.filter(item => item.id !== "software")
      : barcodeCategoryData;

  // ---------- Reset invalid selection ----------
  useEffect(() => {
    if (scanner?.name === "SR 11000" && barcodeCategory?.id === "software") {
      setBarcodeCategory(null);
    }
  }, [scanner]);
 

  const filteredBarcodeTypes = filterBarcodeTypes(
  barcodeTypeData,
  scanner?.barcodeTypeAllowed
);

useEffect(() => {
  if (!scanner || !barcodeType?.id) return;

  const allowed = filteredBarcodeTypes.some(
    t => t.id === barcodeType.id
  );
...
}, [scanner, filteredBarcodeTypes]);
  

  // console.log(barcodeCategory);
                      ...
                      <Row className="mb-3">
                        ...
                        <div className="col-md-10">
                          <Select
                            ...
                            options={filteredBarcodeCategoryData}
                            getOptionLabel={(option) => option?.name || ""}
                            getOptionValue={(option) =>
                              option?.id?.toString() || ""
                            }
